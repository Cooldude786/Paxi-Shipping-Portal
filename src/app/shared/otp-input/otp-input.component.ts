import { ChangeDetectionStrategy, Component, ElementRef, Input, QueryList, ViewChildren, isDevMode } from '@angular/core';
...
export class OtpInputComponent implements ControlValueAccessor, Validator {
  ...
  // NOTE:- in JS '#' is the short syntax to make a var private

  // the private width var is used to get dynamic width of the input boxes
  #width = 50;
  get width() {
    return this.#width
  }

  // the private height var is used to get dynamic width of the input boxes
  #height = 56;
  get height() {
    return this.#height
  }
  ...
  // Since version 3.9.5, TypeScript enforces strictNullChecks on numbers and strings
  // So, I have used any instead of Number for #scheduledFocus below.
  ...
  validate(control: AbstractControl<string, string>): ValidationErrors | null {
    if (!control.value || control.value.length < this.#size) {
      return {
        otpInput: 'Incorrect OTP. Please try again or request another code',
      };
    }
